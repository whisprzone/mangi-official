...
def tracker(restaurant_name):
    try:
        if session["logged_in"]:
            ...
            return render_template("tracker.html", all_restaurants = "Distance between you and the restaurant: " + str(math.ceil(distance_matrix['durations'][0][0] / 60)) + " minutes", data = data, restaurantlist = restaurantlist)
...
def business_signup():
    if request.method == 'POST':
        ...
        email = request.form.get("email")
        password = request.form.get("password")
        if str(fullname) == '' or str(address) == '' or str(phone) == '' or str(email) == '' or str(password) == '':
...
def stocktake_update_waste(indicator):
    return render_template("stocktake.html", indicator=indicator)
